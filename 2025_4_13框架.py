...
# 加载配置文件
config_list = autogen.config_list_from_json("QAI_CONFIG_LIST.json")
...
general_agent = autogen.ConversableAgent(
    ...
    system_message="你是一个通用型 Agent，负责对话开始时基本的信息采集和正常交流对话。根据用户任务和之前的对话历史，生成自然、符合对话上下文的回复，专注于理解用户需求并收集更多信息以支持后续流派选择。如果用户任务不明确，提出澄清问题以引导对话。"
)
...
# 流派 Agents
genre_agents = {
    "CBT": autogen.ConversableAgent(name="CBTAgent", llm_config=llm_config1, system_message="根据用户任务和之前的对话历史，生成神秘风格的文本。"),
    "SFT": autogen.ConversableAgent(name="SFTAgent", llm_config=llm_config1, system_message="根据用户任务和之前的对话历史，生成奇幻风格的文本。"),
    "PA": autogen.ConversableAgent(name="PAAgent", llm_config=llm_config1, system_message="根据用户任务和之前的对话历史，生成科幻风格的文本。"),
    "NT": autogen.ConversableAgent(name="NTAgent", llm_config=llm_config1, system_message="根据用户任务和之前的对话历史，生成浪漫风格的文本。"),
    "HT": autogen.ConversableAgent(name="HTAgent", llm_config=llm_config1, system_message="根据用户任务和之前的对话历史，生成历史风格的文本。")
}
...
def create_evaluators(selected_genres, include_general=False):
    ...
    if include_general:
        evaluators["TheoreticalCoherence_General"] = autogen.AssistantAgent(
            ...
            llm_config=llm_config,
            system_message="根据用户任务和对话历史，评估通用型 Agent 生成文本的理论连贯性（内容是否符合通用对话的基本逻辑和信息采集目标），返回0-5分，格式为：'评分：X分'（X为数字，可含小数）。"
        )
    ...
    evaluators["TechniqueCompatibility"] = autogen.AssistantAgent(
        ...
        llm_config=llm_config,
        system_message="根据用户任务和对话历史，参考理论连贯性和目标一致性分数，评估文本的技术兼容性（内容是否符合心理咨询流派的技术方法，对于通用型 Agent 则评估是否符合信息采集和对话引导逻辑），返回0-5分，格式为：'评分：X分'（X为数字，可含小数）。"
    )
    return evaluators
...
def run_evaluators(results, chat_history, task, conversation_history, genres):
    ...
    for i, result in enumerate(results):
        ...
        try:
            ...
            # 步骤 3：串行执行 TechniqueCompatibility 评估
    ...
    if len(selected_genres) == 1:
        ...
        for genre in selected_genres:
            if genre not in eval_results or not eval_results[genre]:
                ...
                scores[genre] = sum(extract_score(eval["content"]) for eval in eval_results[genre]) / 3
        ...
        def nested_integration(manager, integrator, evaluators, initial_message, chat_history, task, conversation_history):
            ...
            best_version = max(versions, key=lambda x: x["avg_score"]) if versions else {"text": "整合失败，无可用版本", "avg_score": 0}
...
def main():
    ...
    while True:
        ...
        general_text = general_result.chat_history[-1]["content"] if general_result and general_result.chat_history else "通用型 Agent 无输出"
        ...
        if not selected_genres:
            ...
            if genre_results:
                ...
                all_low_score = True
                for genre in selected_genres:
                    if genre in eval_results:
                        scores = [extract_score(eval["content"]) for eval in eval_results[genre]]
                        avg_score = sum(scores) / len(scores) if scores else 0
                        print(f"流派 {genre} 平均评分: {avg_score}")
                        if avg_score >= 3.0:
                            all_low_score = False
                            break
                if all_low_score:
                    final_result = general_text
                    print("所有流派 Agent 评分低于阈值，使用通用型 Agent 输出。")
                    current_chat_history["final"] = [{"content": f"直接输出通用型结果：{final_result} (流派 Agent 评分低)", "role": "assistant"}]
                else:
